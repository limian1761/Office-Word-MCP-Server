...
#### **1.1. 愿景与目标**
本文档旨在设计一个先进的、专为AI调用的Word文档自动化MCP（Model Context Protocol）服务。该服务将取代现有基于简单索引和文本搜索的脆弱定位机制，引入一个能够理解文档深层结构和上下文的**“文档智能操作引擎”**。

通过采用Microsoft Office的COM（Component Object Model）接口作为底层技术，本服务将具备对运行中的Word应用程序进行实时、精细化控制的能力，实现真正的“所见即所得”自动化。
...
#### **1.3. 架构指导原则**
1.  **解耦与模块化:** 各组件职责单一、高度独立，便于独立开发、测试和维护。
2.  **声明式定位:** AI调用者只需通过`Locator`对象**声明**“要找什么”，而无需关心**如何找到**它的具体算法。
3.  **抽象化后端:** 必须将所有与`pywin32`相关的COM对象操作严格封装在后端适配层，防止COM的复杂性泄漏到上层业务逻辑中。
4.  **可扩展性:** 架构应能轻松支持新的文档元素类型（图表、形状）、新的定位策略和新的操作。
