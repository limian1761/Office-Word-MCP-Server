        ...
        for i in range(1, comments_count + 1):
            try:
                ...
                try:
                    ...
                    print(
                        f"Warning: Failed to retrieve comment information for index {i}: {e}"
                    )
                    continue
            except Exception as e:
...
def get_comments_by_range(
    backend: WordBackend, com_range_obj: win32com.client.CDispatch
) -> List[Dict[str, Any]]:
    """
        ...
        for i in range(1, comments_count + 1):
            try:
                ...
                try:
                    ...
                    if (
                        hasattr(comment, "Scope")
                        and hasattr(comment.Scope, "Start")
                        and hasattr(comment.Scope, "End")
                        and comment.Scope.Start >= com_range_obj.Start
                        and comment.Scope.End <= com_range_obj.End
                    ):
                        comment_info = {
                            "index": i - 1,  # 0-based index
                            "text": (
                                comment.Range.Text if hasattr(comment, "Range") else ""
                            ),
                            "author": (
                                comment.Author
    ...
    try:
        ...
        if not hasattr(backend.document, "Comments"):
            raise WordDocumentError(
                "Comments collection is not available in this document."
            )
        ...
        if comment_index < 0 or comment_index >= comments_count:
            raise ValueError(
                f"Invalid comment index: {comment_index}. Valid range is 0 to {comments_count - 1}."
            )
...
def reply_to_comment(
    backend: WordBackend, comment_index: int, reply_text: str, author: str = "User"
) -> None:
    """
    ...
    try:
        ...
        reply = backend.document.Comments.Add(Range=comment.Scope, Text=reply_text)
...
def get_comment_thread(backend: WordBackend, comment_index: int) -> Dict[str, Any]:
    ...
    try:
        ...
        for i in range(1, backend.document.Comments.Count + 1):
            ...
            if (
                comment.Scope.Start == original_scope.Start
                and comment.Scope.End == original_scope.End
            ):
