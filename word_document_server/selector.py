    ...
    def _get_initial_candidates(self, backend: WordBackend, element_type: str, search_range: Optional[Any] = None) -> List[Any]:
        ...
        if search_range:
            ...
            if element_type == "paragraph":
                ...
                candidates = [p for p in all_paragraphs if p.Style.NameLocal.startswith("Heading") or p.Style.NameLocal.startswith("标题")]
            elif element_type == "cell":
        ...
        if not candidates and element_type not in ["paragraph", "table", "heading", "cell", "run"]:
