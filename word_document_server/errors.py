...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("word_document_server.log"),
...
class StyleNotFoundError(WordDocumentError):
    ...
    def __init__(self, style_name: str, message: Optional[str] = None, similar_styles: Optional[list] = None):
...
def handle_error(e: Exception) -> Tuple[int, str, Dict[str, Any]]:
    ...
    # Handle common exceptions
        ...
        return ErrorCode.DOCUMENT_OPEN_ERROR.value[0], f"File not found: {str(e)}", {"file_path": str(e.filename) if hasattr(e, 'filename') else None}
    
    elif isinstance(e, ValueError):
        return ErrorCode.INVALID_INPUT.value[0], f"Invalid value: {str(e)}", {"error_type": "ValueError"}
    
    elif isinstance(e, PermissionError):
        return ErrorCode.PERMISSION_DENIED.value[0], f"Permission denied: {str(e)}", {"error_type": "PermissionError"}
    
    # Handle general exceptions
    else:
        return ErrorCode.SERVER_ERROR.value[0], f"An unexpected error occurred: {str(e)}", {"error_type": type(e).__name__}
...
def validate_input_params(params: Dict[str, Any], required_params: list) -> Tuple[bool, str]:
    """
