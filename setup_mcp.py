...
def generate_mcp_config_local():
    ...
    config = {
        "mcpServers": {
            "word_docx_tools": {"command": sys.executable, "args": [server_script_path]}
        }
    }
...
def install_from_pypi():
    ...
    try:
        subprocess.run(
            [sys.executable, "-m", "pip", "install", "word_docx_tools"], check=True
        )
...
def get_transport_choice():
    ...
    if choice == "2":
        ...
        return {"transport": "sse", "host": host, "port": port, "sse_path": sse_path}
    else:
        # Default to stdio
...
def setup_venv():
    ...
    try:
        ...
        if os.path.exists(requirements_path):
            subprocess.run(
                [sys.executable, "-m", "uv", "pip", "install", "-r", requirements_path],
                check=True,
            )
        else:
...
def generate_mcp_config_local(python_path, transport_config):
    ...
    env = {"PYTHONPATH": base_path, "MCP_TRANSPORT": transport_config["transport"]}
...
def generate_mcp_config_uvx(transport_config):
    ...
    # Build environment variables
    env = {"MCP_TRANSPORT": transport_config["transport"]}
...
def install_from_pypi():
    ...
    try:
        subprocess.run(
            [sys.executable, "-m", "pip", "install", "word-docx-tools"], check=True
        )
...
def print_config_instructions(config_path, transport_config):
    ...
    if transport_config["transport"] == "streamable-http":
        ...
        print(
            f"   Server will be accessible at: http://{transport_config['host']}:{transport_config['port']}{transport_config['path']}"
        )
        print(f"   \n   To test the server manually:")
        print(
            f"   curl -X POST http://{transport_config['host']}:{transport_config['port']}{transport_config['path']}"
        )

    elif transport_config["transport"] == "sse":
        print(f"\n📡 SSE Transport Configuration:")
        print(
            f"   Server will be accessible at: http://{transport_config['host']}:{transport_config['port']}{transport_config['sse_path']}"
        )
        print(f"   \n   To test the server manually:")
        print(
            f"   curl http://{transport_config['host']}:{transport_config['port']}{transport_config['sse_path']}"
        )
...
if __name__ == "__main__":
    # Check prerequisites
    python_ok, uv_installed, uvx_installed, word_server_installed = (
        check_prerequisites()
    )
    ...
    if word_server_installed:
        ...
        if uvx_installed:
            ...
            if choice == "1":
                ...
                config_path = generate_mcp_config_local()
                print_config_instructions(config_path, transport_config)
            else:
