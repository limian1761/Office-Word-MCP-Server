...
class TestRealE2E(unittest.TestCase):
    ...
    def test_document_operations_with_locator(self):
        ...
        result = document_tools(
            ctx=self.context, operation_type="open", file_path=self.test_doc_path
        )
        self.assertIn("successfully", result.lower())

        # 2. 获取文档样式
        result = document_tools(ctx=self.context, operation_type="get_styles")
        # 验证返回的是有效的JSON格式样式列表
        ...
        # 3. 获取文档元素
        result = document_tools(
            ctx=self.context, operation_type="get_objects", object_type="paragraphs"
        )
        # 验证返回的是有效的JSON格式元素列表
        ...
        # 4. 关闭文档
        result = document_tools(ctx=self.context, operation_type="close")
        self.assertIn("successfully", result.lower())
    ...
    def test_text_operations_with_locator(self):
        ...
        # 2. 查找文本
        result = text_tools(ctx=self.context, operation_type="find", text="段落")
        # 验证返回的是有效的JSON格式查找结果
        ...
        # 3. 关闭文档
        document_tools(ctx=self.context, operation_type="close")
    ...
    def test_table_operations_with_locator(self):
        ...
        # 2. 创建表格
        result = table_tools(ctx=self.context, operation_type="create", rows=3, cols=3)
        self.assertIn("successfully", result.lower())
    ...
    def test_comment_operations_with_locator(self):
        ...
        # 3. 获取所有注释
        result = comment_tools(ctx=self.context, operation_type="get_all")
        # 验证返回的是有效的JSON格式注释列表
