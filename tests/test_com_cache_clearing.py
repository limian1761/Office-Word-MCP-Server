...
class TestComCacheClearing(unittest.TestCase):
    ...
    @patch('word_document_server.utils.app_context.win32com.client.Dispatch')
    ...
    @patch('word_document_server.utils.app_context.win32com.client.Dispatch')
    @patch('word_document_server.utils.app_context.AppContext._clear_com_cache')
    def test_get_word_app_with_com_error_and_cache_clear_success(self, mock_clear_cache, mock_dispatch, mock_reload):
    ...
    @patch('word_document_server.utils.app_context.win32com.client.Dispatch')
    @patch('word_document_server.utils.app_context.AppContext._clear_com_cache')
    def test_get_word_app_with_com_error_and_cache_clear_failure(self, mock_clear_cache, mock_dispatch):
    ...
    @patch('word_document_server.utils.app_context.win32com.client.Dispatch')
    @patch('word_document_server.utils.app_context.AppContext._clear_com_cache')
    def test_get_word_app_with_com_error_and_retry_failure(self, mock_clear_cache, mock_dispatch, mock_reload):
        """Test Word application creation with COM error and failed retry after cache clearing"""
        # Mock AttributeError on both calls
        mock_dispatch.side_effect = [AttributeError("COM error"), AttributeError("COM error after cache clear")]
    ...
    @patch('word_document_server.utils.app_context.os.path.exists')
    @patch('word_document_server.utils.app_context.shutil.rmtree')
    @patch('word_document_server.utils.app_context.win32com')
