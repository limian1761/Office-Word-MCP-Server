...
|---------|---------|------|--------------|
| [1001] | 参数错误 | 提供的参数不符合要求 | 检查参数格式和值是否正确 |
| [2001] | 文档错误 | 没有活动文档或文档无法访问 | 确保已通过`open_document`打开有效文档 |
| [2002] | 元素未找到 | 无法找到符合条件的元素 | 检查Locator定位器是否正确 |
| [2003] | 定位器歧义 | 定位器匹配到多个元素 | 提供更精确的过滤条件 |
| [4001] | 操作失败 | 操作无法完成 | 检查文档状态和权限 |
| [4002] | 格式错误 | 提供的格式设置无效 | 检查格式参数是否符合要求 |
| [4003] | COM组件错误 | Word COM组件访问错误（如"Open.Paragraphs"） | 确保文档结构完整，可能需要重启Word应用 |
| [4004] | 特殊组件错误 | 特定Word组件访问错误（如"Open.Comments"） | 确保文档包含相应组件，可能需要重启Word应用 |
| [7001] | 表格错误 | 表格操作失败 | 检查文档结构是否完整，尝试关闭并重新打开文档 |
| [8001] | 注释错误 | 注释操作失败 | 检查文档是否包含注释组件，可能需要重启Word应用 |
