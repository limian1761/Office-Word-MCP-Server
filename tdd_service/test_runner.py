"""
Automated Test Runner for TDD Service.

This module provides tools for automatically running tests and monitoring results.
"""
...
# Configure logging
logger = logging.getLogger('TDDTestRunner')
...
def tdd_test_runner(
    ...
    stop_on_success: bool = Field(
        default=True,
        description="Stop testing once all tests pass (default: True)"
    )
) -> dict:
    """
    ...
    log_info(f"Starting TDD test runner with command: {test_command}")
    ...
    summary = {
        ...
        "message": f"TDD test runner completed after {attempt} attempts",
    ...
    log_info(f"TDD test runner finished. Final status: {summary['status']}")
    return summary